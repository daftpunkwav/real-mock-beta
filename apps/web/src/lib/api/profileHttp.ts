/** 档案 / 简历 / 处理器配置 REST 客户端（能力命名，非后端包名）。 */
...
import type { Resume, ResumeActivateResponse } from "@/types";
...
export const profileHttp = {
  ...
  /* 档案 */
  getProfile: () => request<UserProfileResponse>("/v1/profile"),
  updateProfile: (data: Partial<UserProfileResponse>) =>
    request<UserProfileResponse>("/v1/profile", { method: "PUT", body: JSON.stringify(data) }),
