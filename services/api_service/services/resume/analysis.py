"""简历深度评价逻辑（LLM 评价 + 联网检索规划 + 结果规范化）。

从 ``api_service/routes/resume.py`` 拆出，路由层只保留上传 / CRUD / 薄调用，
评价的 prompt、检索词规划、LLM 调用与容错规范化集中在本模块，可独立单测。
"""
...
async def _generate_market_queries(r: Resume, llm: LLMClient) -> tuple[list[str], bool]:
    ...
    try:
        ...
        if isinstance(raw, list):
            cleaned = [str(q).strip()[:40] for q in raw if str(q).strip()][:_MAX_SEARCH_QUERIES]
            if cleaned:
                return cleaned, True
...
async def analyze_resume_with_llm(r: Resume, db: Session) -> ResumeAnalysis:
    ...
    """
    # 简历评价：默认对话模型 + 最大思考强度（条目未声明思考能力时自动忽略）
